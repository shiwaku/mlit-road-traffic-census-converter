...
- **変換/連携: 実装済み（案1=外部ルックアップ方式で確定）** — `census_converter/jikantai.py`（`run.py` の
  `--step jikantai`）が zkntrf → 県別JSON（`data/{year}/output/jikantai/{NN}.json` + `index.json`）を生成。
  各区間 `{"<都道府県指定市コード>_<交通量調査単位区間番号>": {up/down:{s,l}, t12, t24}}`。時間帯は
  表の7時始まり24列を **0–23時の時系列順に並べ替えて格納**。t12/t24 は車種別に上下合算。
  - **全国結合率（実測）**: R03 97.0% / H27 98.2%（feature加重）。都道府県指定市コードは単一zkntrfファイルにのみ
    出現するため、地物のコードから読むべき県別JSONが一意に決まる（`NN = コード // 1000`、全81コードで検証済み）。
  - **PMTilesは無改変**。結合キー列（R03=`交通量／…`, H27=`交通量調査単位区間番号／…`）が
    PMTiles(z14)に残存することを tippecanoe-decode で確認済み。
  - **ビューワ連携: 実装済み** — クリック時に地物のコード→県別JSONを遅延fetch→複合キーで引き、
    上り/下り(全車)の折れ線グラフをインラインSVG（依存無し）でポップアップに描画。
    多くの区間は12時間観測で夜間は欠測(null)＝線が切れて日中のみ表示、t24は空でt12を表示。
    関連実装: `viewer/src/{data/jikantai.ts, ui/chart.ts, ui/popup.ts, config/years.ts}`、`viewer/vite.config.ts`（開発配信）。
